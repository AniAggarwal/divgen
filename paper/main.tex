...
% ---- result macros (filled from the full 552-prompt GenEval run) ----
\newcommand{\dinoIID}{0.588}      % paper i.i.d.
\newcommand{\dinoParmar}{0.705}
\newcommand{\dinoGrad}{0.784}
\newcommand{\dsGrad}{0.411}
\newcommand{\lpipsGrad}{0.767}
\newcommand{\clipGrad}{0.349}
% ours (placeholders -> final numbers)
\newcommand{\dinoOursInit}{0.627}
\newcommand{\dinoOurs}{0.794}
\newcommand{\dsOursInit}{0.308}
\newcommand{\dsOurs}{0.473}
\newcommand{\lpipsOursInit}{0.642}
\newcommand{\lpipsOurs}{0.729}
\newcommand{\vendiOursInit}{1.98}
\newcommand{\vendiOurs}{2.81}
\newcommand{\clipOursInit}{0.336}
\newcommand{\clipOurs}{0.350}
\newcommand{\numPrompts}{10}
...
schedules. On GenEval with SDXL-Turbo, breeding reaches approximate parity with
the paper's gradient-based optimization on its own metrics --- without a single
backward pass through the sampler. We describe what the population view adds
...
Three ideas in Harrington et al.\ strike us as durable. First, that
...
supplied by us (Fig.~\ref{fig:conv}, right). A controlled comparison (Tab.~\ref{tab:mut}) confirms
the fixed-rate pathologies transfer to this domain and that the self-tuned
variant matches the best fixed rate without knowing it in advance.
...
rows from Harrington et al.\ Tab.~1 (their eval harness, 552 prompts); our rows
use the official repository's own metric code over \numPrompts{} prompts
(\emph{preliminary; the full 552-prompt run is in progress and will replace
these numbers}). Diversity metrics: higher is better.}
\label{tab:main}
\footnotesize
...
\midrule
i.i.d. & \dinoIID & 0.249 & 0.642 & 0.335 \\
Parmar et al.~\cite{parmar2025scaling} & \dinoParmar & 0.331 & 0.682 & 0.333 \\
...
being traded away. On the diversity metrics, breeding lands at approximate
parity with the gradient optimizer --- slightly ahead on DINO and DreamSim,
slightly behind on LPIPS --- despite never computing
...
\centering
\small
\caption{Mutation-rate comparison (4 GenEval prompts, 45 generations, matched
seeds), echoing our ECAD Table~9 in this new domain. The self-tuned variant
needs no rate chosen in advance.}
\label{tab:mut}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lcccc@{}}
...
fixed rate $15\%$ & \textbf{0.774} & 0.369 & 2.15 & 0.784 \\
self-tuned & 0.756 & \textbf{0.374} & \textbf{2.25} & \textbf{0.792} \\
\bottomrule
\end{tabular}
\end{table}
...
$0.65$--$0.69$ DINO diversity on prompt families they never saw during
...
reaches the highest raw diversity we observe (DINO $0.886$) at a real cost in
prompt fidelity --- it maps the far end of the quality--diversity front --- and
its early winners rediscover low-frequency structure on their own, a pleasing
...
applies: at $0.345$\,s per iteration on an A100 (their Sec.~4.2), it reaches its
...
This note exists because the original paper's analysis was clear enough to
build on in a week: the frequency finding told us what mutations to propose,
the norm regularizer told us what constraint to repair to, and the set-level
metrics told us what to select for. That is what a good paper does. We offer
the population view back as a companion --- same landscape, second vehicle ---
with code, configurations, and scaling scripts in the fork above, in the hope
it is useful to the authors' future work on collapse recovery.
